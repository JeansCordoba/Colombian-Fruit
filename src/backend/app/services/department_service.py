...
class DepartmentService:
    ...
    def create_department(department):
        with get_session() as session:
            ...
            return DepartmentService._serialize_department(new_department)
    
    @staticmethod
    def get_all_departments():
        with get_session() as session:
            departments = session.exec(select(Department)).all()
            return [DepartmentService._serialize_department(department) for department in departments]
    ...
    def get_department_by_id(department_id: int):
        with get_session() as session:
            ...
            return DepartmentService._serialize_department(department)
    ...
    def search_departments(search):
        with get_session() as session:
            ...
            if search.name:
                normalized_name = Utilities.remove_accents(search.name.lower())
            ...
            if search.region_name:
                normalized_region_name = Utilities.remove_accents(search.region_name.lower())
